...
Normal3d::Normal3d(Vertex3d vertex3d1,Vertex3d vertex3d2,Vertex3d vertex3d3){
	Edge3d edge1 = Edge3d(vertex3d1,vertex3d2);
	Edge3d edge2 = Edge3d(vertex3d1,vertex3d3);

	Normal3d normal_edge1 = Normal3d(edge1);
	Normal3d normal_edge2 = Normal3d(edge2); 

	Normal3d normal = Normal3d(normal_edge1,normal_edge2);
	i = normal.i;
	j = normal.j;
	k = normal.k; 
}	
...
Surface3d::Surface3d(Edge3d edge3d1,Edge3d edge3d2,Vertex3d vertex){
	Normal3d normal1 = Normal3d(edge3d1);
	Normal3d normal2 = Normal3d(edge3d2);
	Normal3d normal = Normal3d(normal1,normal2);
...
void Surface_List::removeSurface(Surface3d s){
...
bool Surface_List::checkSurface(Surface3d s){
	bool result=false;
	for(int i=0;i<S.size();i++){
		if(equal_3dsurface(S[i], s)){
			result=true;
...
bool equal_3dsurface(Surface3d surface1, Surface3d surface2){
	...
	if(equal_normals(surface1.normal_vector,surface2.normal_vector)){
		if(product_of_normal_vertex_in3d(surface1.normal_vector,surface1.surface_vertex)==product_of_normal_vertex_in3d(surface2.normal_vector,surface2.surface_vertex)){
			temp = true;
		}
	}
	return temp;
...
int dot_product_of_normals(Normal3d normal1,Normal3d normal2){
